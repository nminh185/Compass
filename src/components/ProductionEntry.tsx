...
export function ProductionEntry() {
  ...
  const { t } = useLanguage();
  const { addOrder, error } = useProductionStore();
  ...
  return (
    <div className="max-w-2xl mx-auto p-6">
      <div className="bg-white rounded-lg shadow-sm p-6">
        <h2 className="text-2xl font-semibold text-gray-900 mb-6">{t('production.orderEntry')}</h2>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div>
            ...
            <input
              ...
              type="text"
              className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-[#0070f2] focus:ring-[#0070f2] sm:text-sm"
              placeholder={t('production.employeeId')}
            />
            {errors.employeeId && (
